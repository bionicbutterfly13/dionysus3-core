...
-- ThoughtSeed layer hierarchy (5 levels)
CREATE TYPE thoughtseed_layer AS ENUM (
    'sensorimotor',  -- Layer 1: Direct perception/action
    'perceptual',    -- Layer 2: Pattern recognition
    'conceptual',    -- Layer 3: Abstract concepts
    'abstract',      -- Layer 4: Higher-order reasoning
    'metacognitive'  -- Layer 5: Self-reflection
);

-- Basin states for attractor dynamics
CREATE TYPE basin_state AS ENUM (
    'dormant',       -- Below activation threshold
    'activating',    -- Crossing threshold
    'active',        -- Fully engaged
    'saturated',     -- At maximum capacity
    'decaying',      -- Post-activation decline
    'inhibited'      -- Suppressed by competing basin
);

-- Basin types for categorical organization
CREATE TYPE basin_type AS ENUM (
    'conceptual',    -- Abstract concept attractors
    'emotional',     -- Emotional state attractors
    'semantic',      -- Meaning-based attractors
    'pattern',       -- Recurring pattern attractors
    'memory',        -- Memory consolidation attractors
    'attention',     -- Focus/attention attractors
    'consciousness', -- Meta-awareness attractors
    'hybrid'         -- Multi-type attractors
);
...
-- Active Inference State (Prediction Error Minimization)
CREATE TABLE active_inference_states (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    created_at TIMESTAMPTZ DEFAULT NOW(),

    -- Core active inference metrics
    prediction_error FLOAT NOT NULL DEFAULT 0.0,
    free_energy FLOAT NOT NULL DEFAULT 0.0,
    surprise FLOAT NOT NULL DEFAULT 0.0,
    precision FLOAT NOT NULL DEFAULT 1.0,

    -- Hierarchical beliefs (JSONB for flexibility)
    beliefs JSONB NOT NULL DEFAULT '{}',
    prior_beliefs JSONB NOT NULL DEFAULT '{}',
    prediction_updates JSONB NOT NULL DEFAULT '{}',

    -- Link to memory that triggered this state
    memory_id UUID REFERENCES memories(id) ON DELETE SET NULL,

    -- Consciousness level at time of state
    consciousness_level TEXT CHECK (consciousness_level IN (
        'minimal', 'basic', 'functional', 'full'
    ))
);

-- IWMT Coherence Tracking (Integrated World Modeling Theory)
CREATE TABLE iwmt_coherence (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    created_at TIMESTAMPTZ DEFAULT NOW(),

    -- Coherence metrics (0.0 to 1.0)
    spatial_coherence FLOAT NOT NULL DEFAULT 0.0
        CHECK (spatial_coherence >= 0 AND spatial_coherence <= 1),
    temporal_coherence FLOAT NOT NULL DEFAULT 0.0
        CHECK (temporal_coherence >= 0 AND temporal_coherence <= 1),
    causal_coherence FLOAT NOT NULL DEFAULT 0.0
        CHECK (causal_coherence >= 0 AND causal_coherence <= 1),

    -- Embodied selfhood metrics
    embodied_selfhood FLOAT NOT NULL DEFAULT 0.0
        CHECK (embodied_selfhood >= 0 AND embodied_selfhood <= 1),
    autonomous_agency FLOAT NOT NULL DEFAULT 0.0
        CHECK (autonomous_agency >= 0 AND autonomous_agency <= 1),

    -- Counterfactual modeling
    counterfactual_capacity FLOAT NOT NULL DEFAULT 0.0
        CHECK (counterfactual_capacity >= 0 AND counterfactual_capacity <= 1),
    counterfactual_scenarios JSONB DEFAULT '[]',

    -- Consciousness achievement
    consciousness_level FLOAT NOT NULL DEFAULT 0.0
        CHECK (consciousness_level >= 0 AND consciousness_level <= 1),
    consciousness_achieved BOOLEAN DEFAULT FALSE,
    consciousness_quality FLOAT NOT NULL DEFAULT 0.0
        CHECK (consciousness_quality >= 0 AND consciousness_quality <= 1),

    -- Link to identity
    identity_aspect_id UUID REFERENCES identity_aspects(id)
);

-- ThoughtSeeds (5-Layer Cognitive Hierarchy)
CREATE TABLE thoughtseeds (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    created_at TIMESTAMPTZ DEFAULT NOW(),

    -- Core identity
    layer thoughtseed_layer NOT NULL,
    activation_level FLOAT DEFAULT 0.0
        CHECK (activation_level >= 0 AND activation_level <= 1),

    -- Neuronal packet (cognitive content)
    neuronal_packet JSONB NOT NULL DEFAULT '{}',

    -- Evolutionary prior (subpersonal processing)
    evolutionary_prior JSONB DEFAULT '{}',

    -- Competition dynamics
    competition_status TEXT DEFAULT 'pending'
        CHECK (competition_status IN ('pending', 'competing', 'won', 'lost', 'merged')),
    winner_id UUID REFERENCES thoughtseeds(id),
    ...
    consciousness_contribution FLOAT DEFAULT 0.0
        CHECK (consciousness_contribution >= 0 AND consciousness_contribution <= 1)
);

-- ThoughtSeed Competition History
CREATE TABLE thoughtseed_competitions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    competition_time TIMESTAMPTZ DEFAULT NOW(),
    ...
    -- Energy dynamics
    competition_energy FLOAT NOT NULL,
    energy_transfers JSONB DEFAULT '{}',
...
-- Consciousness Events (Audit Trail)
CREATE TABLE consciousness_events (
    ...
    event_type TEXT NOT NULL CHECK (event_type IN (
        'activation',       -- Basin/thoughtseed activated
        'deactivation',     -- Basin/thoughtseed deactivated
        'transition',       -- State transition
        'coherence_shift',  -- IWMT coherence change
        'belief_update',    -- Active inference belief update
        'consolidation'     -- Memory consolidation event
    )),

    -- State snapshots
    active_inference_state_id UUID REFERENCES active_inference_states(id),
    iwmt_coherence_id UUID REFERENCES iwmt_coherence(id),

    -- Basin state at event
    active_basins UUID[],
...
-- ============================================================================
-- BASIN EXTENSIONS TO memory_clusters
-- ============================================================================

-- Add basin-specific columns to existing memory_clusters table
ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    basin_type basin_type DEFAULT 'semantic';

ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    basin_state basin_state DEFAULT 'dormant';

ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    activation_threshold FLOAT DEFAULT 0.5;

ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    current_activation FLOAT DEFAULT 0.0;
...
-- CLAUSE basin strengthening (+0.2 per activation, cap at 2.0)
ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    clause_strength FLOAT DEFAULT 1.0
    CHECK (clause_strength >= 1.0 AND clause_strength <= 2.0);

-- Co-occurring concepts for CLAUSE scoring
ALTER TABLE memory_clusters ADD COLUMN IF NOT EXISTS
    co_occurring_concepts JSONB DEFAULT '{}';

-- ============================================================================
-- NEURAL FIELD EXTENSIONS TO cluster_relationships
-- ============================================================================

ALTER TABLE cluster_relationships ADD COLUMN IF NOT EXISTS
    field_contribution FLOAT DEFAULT 0.0;

ALTER TABLE cluster_relationships ADD COLUMN IF NOT EXISTS
    oscillation_frequency FLOAT DEFAULT 0.0;

ALTER TABLE cluster_relationships ADD COLUMN IF NOT EXISTS
    phase_coupling FLOAT DEFAULT 0.0
    CHECK (phase_coupling >= -1 AND phase_coupling <= 1);

-- ============================================================================
-- CONSCIOUSNESS FUNCTIONS
-- ============================================================================

-- Activate a basin with given strength
CREATE OR REPLACE FUNCTION activate_basin(
    p_cluster_id UUID,
    p_activation_strength FLOAT
) RETURNS TABLE (
    new_state basin_state,
    new_activation FLOAT,
    new_clause_strength FLOAT
) AS $$
DECLARE
    v_threshold FLOAT;
    v_current FLOAT;
...
BEGIN
    -- Get current state
    SELECT activation_threshold, current_activation, basin_state, clause_strength
    INTO v_threshold, v_current, v_state, v_clause
    FROM memory_clusters WHERE id = p_cluster_id;

    -- Update activation
    v_current := LEAST(1.0, v_current + p_activation_strength);

    -- State machine transitions
    IF v_state = 'dormant' AND v_current >= v_threshold THEN
        v_state := 'activating';
    ELSIF v_state = 'activating' AND v_current >= v_threshold + 0.2 THEN
        v_state := 'active';
    ELSIF v_state = 'active' AND v_current >= 0.95 THEN
        v_state := 'saturated';
    END IF;

    -- CLAUSE strengthening: +0.2 per activation, cap at 2.0
    v_clause := LEAST(2.0, v_clause + 0.2);

    -- Update basin
    UPDATE memory_clusters
    SET current_activation = v_current,
        basin_state = v_state,
        clause_strength = v_clause,
        activation_count = activation_count + 1,
        last_activated = NOW()
    WHERE id = p_cluster_id;

    RETURN QUERY SELECT v_state, v_current, v_clause;
END;
$$ LANGUAGE plpgsql;

-- Update active inference state with prediction error calculation
CREATE OR REPLACE FUNCTION update_active_inference(
    p_memory_id UUID,
    p_prediction JSONB,
    p_observation JSONB
) RETURNS UUID AS $$
...
BEGIN
    -- Calculate prediction error (simplified Euclidean in JSON space)
    -- In production, this would use embedding distance
    v_pred_error := COALESCE(
        (SELECT AVG(ABS((p_prediction->>key)::float - (p_observation->>key)::float))
         FROM jsonb_object_keys(p_prediction) AS key
         WHERE p_observation ? key),
        0.5
    );

    -- Free energy = prediction error + complexity penalty
    v_free_energy := v_pred_error + 0.01 * (
        SELECT COUNT(*) FROM jsonb_object_keys(p_prediction)
    );

    -- Surprise = negative log probability (bounded)
    v_surprise := -ln(1.0 - LEAST(0.99, v_pred_error));

    INSERT INTO active_inference_states (
        memory_id, prediction_error, free_energy, surprise, precision,
        beliefs, prior_beliefs
    ) VALUES (
        p_memory_id, v_pred_error, v_free_energy, v_surprise, v_precision,
        p_observation, p_prediction
...
-- Calculate overall IWMT consciousness level
...
BEGIN
    -- Weighted combination per IWMT theory
    -- Spatial, temporal, causal coherence are primary
    -- Embodied selfhood and counterfactual capacity are modulators
    RETURN (
        (p_spatial * 0.25) +
        (p_temporal * 0.25) +
        (p_causal * 0.25) +
        (p_embodied * 0.15) +
        (p_counterfactual * 0.10)
    );
END;
...
CREATE INDEX idx_active_inference_consciousness
    ON active_inference_states(consciousness_level);
CREATE INDEX idx_active_inference_prediction_error
    ON active_inference_states(prediction_error);
CREATE INDEX idx_active_inference_memory
    ON active_inference_states(memory_id);

CREATE INDEX idx_clusters_basin_state
    ON memory_clusters(basin_state) WHERE basin_state = 'active';
CREATE INDEX idx_clusters_clause_strength
    ON memory_clusters(clause_strength DESC);

CREATE INDEX idx_thoughtseeds_layer
    ON thoughtseeds(layer, activation_level DESC);
CREATE INDEX idx_thoughtseeds_competition
    ON thoughtseeds(competition_status) WHERE competition_status = 'competing';

CREATE INDEX idx_consciousness_events_time
    ON consciousness_events(event_time DESC);
CREATE INDEX idx_consciousness_events_type
    ON consciousness_events(event_type);

CREATE INDEX idx_iwmt_coherence_level
    ON iwmt_coherence(consciousness_level DESC);

-- ============================================================================
-- CONSCIOUSNESS COMMENTS
-- ============================================================================

COMMENT ON TABLE active_inference_states IS 'Tracks prediction error minimization state per Karl Friston active inference framework.';

COMMENT ON TABLE iwmt_coherence IS 'Integrated World Modeling Theory coherence metrics per Adam Safron IWMT framework.';

COMMENT ON TABLE thoughtseeds IS '5-layer cognitive hierarchy from sensorimotor to metacognitive processing.';

COMMENT ON TABLE thoughtseed_competitions IS 'Audit log of thoughtseed winner selection within each cognitive layer.';

COMMENT ON TABLE consciousness_events IS 'Event sourcing table for consciousness state changes and transitions.';

COMMENT ON FUNCTION activate_basin IS 'Activates attractor basin with CLAUSE strengthening (+0.2 per activation, 2.0 cap).';

COMMENT ON FUNCTION update_active_inference IS 'Creates active inference state with prediction error calculation.';

COMMENT ON FUNCTION calculate_consciousness_level IS 'IWMT consciousness level from component coherence metrics.';